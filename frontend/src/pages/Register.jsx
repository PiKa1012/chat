...
export default function Register() {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      await axios.post(`${config.API_URL}/api/auth/register`, { username, password })
      navigate('/')
    } catch (err) {
      ...
      <div style={{ minHeight: '100vh', background: C.bg, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ width: 360, background: '#fff', borderRadius: 20, padding: '40px 36px', boxShadow: '0 8px 32px rgba(0,0,0,0.10)' }}>
          <div style={{ textAlign: 'center', marginBottom: 32 }}>
            ...
            <h1 style={{ fontSize: 24, fontWeight: 800, color: C.text, marginBottom: 6 }}>创建账号</h1>
            <p style={{ fontSize: 13, color: C.textMuted }}>加入 MyChat，开始聊天</p>
          </div>
          ...
          <p style={{ textAlign: 'center', marginTop: 24, fontSize: 13, color: C.textMuted }}>
            已有账号？{' '}
            <Link to="/" style={{ color: C.accent, fontWeight: 600, textDecoration: 'none' }}>立即登录</Link>
          </p>
        </div>
      </div>
    </>
